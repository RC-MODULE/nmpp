...
void nmppsLog_32f (const nm32f *pSrcVec, nm32f *pDstVec, int nSize)
{
	...
	while (nSize>0){
		...
		//	„тобы пон€ть код ниже, см. log_vdc.cpp, дл€ которого есть
		//	реализаци€ того же алгоритма без ассемблерной магии
...
//	slow !!!	".int 0xf7990000						\n\t"
		...
		//	≈сли аргумент мал (0.7 > х), домножаем на 2^126
		asm ( 	ALL_FPU (".float vreg1 - .retrive(vreg5), set mask if <;")
				...
				ALL_FPU (".float vreg7= mask ?   vreg7 + .retrive(vreg5) : vreg7;")
					: "+a" (cfs) );	//	пров€зываем инструкции зависимост€ми
		int i;
		//	ƒвигаем аргумент к единице (сверху)
		//	Ѕыло бы достаточно (i=0; i<13; i++), но в vreg5 не хватает места
		for (i=0; i<9; i++){
			asm ( 	ALL_FPU (".float vreg1 - .retrive(vreg5), set mask if >;")
					ALL_FPU_ANTI_MASK
					ALL_FPU (".float vreg1= mask ?   vreg1 * .retrive(vreg5) : vreg1;")
		...
		//	ƒвигаем аргумент к единице, теперь снизу (дл€ 0.7 < х < 1)
		for (i=0; i<1; i++){
		...
		asm ( 	ALL_FPU (".float vreg7= vreg7 * .retrive(vreg6);")//  res * log(2)
			    ...
			    //  Taylor
			    //  x^2 x^3 и т.д. не прибавл€ем сразу, а придерживаем,
			    //  чтобы складывать от меньших к большим,- дл€ лучшей точности
