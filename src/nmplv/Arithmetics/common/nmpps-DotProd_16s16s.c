	...
	)
{
		...
		int ret = aura_call(n, "nmppsDotProd_16s16s", &retbuf,  iobuf_src0,iobuf_src1, size); 
		if (ret != 0) {
			printf ("bug = %d\r\n",ret);
			BUG(n, "Call nmppsDotProd_16s16s failed!"); }
		*dst = aura_buffer_get_u64(retbuf);
		aura_buffer_release(n, iobuf_src0); 
		aura_buffer_release(n, iobuf_src1); 
		aura_buffer_release(n, retbuf); 
		slog(0, SLOG_INFO, "ARM: Call nmppsDotProd_16s16s -ok"); 
	#else
		Tmp2BuffSpec s;
		Tmp2BuffSpec* spec=&s;
		spec->buffer0=nmppsMalloc_64s(size);
		if (nmppsMallocSuccess()){
			nmppsConvert_16s64s(srcVec1,(nm64s*)spec->buffer0,size);
