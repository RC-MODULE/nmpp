...
extern "C" {
	...
	int nmppsFFT2048InvInitAllocCustom(  NmppsFFTSpec** specFFT, Malloc32Func* allocate, Free32Func* free, int settings)
	{
		...
		*specFFT = spec;
		if (spec==0) return -1;
		spec->buffer[0]=allocate(2048*2*3);
		spec->buffer[1]=allocate(2048*2*3);
		spec->buffer[2]=0;
		spec->buffer[3]=0;
		spec->shift [0]=11;
		spec->shift [1]=-1;
	...
	int nmppsFFT2048InvInitAlloc( NmppsFFTSpec** spec, void* src, void* dst, int settings)
	{
		...
		*spec = (NmppsFFTSpec*) aura_buffer_get_u32(retbuf);
		...
		if (settings&NMPP_OPTIMIZE_DISABLE){}
			...
			fseq64 allocRoute;
			ret=nmppsFFT2048InvOptimize(src, dst, &allocRoute);
		...
		*allocOrder=0xFFFFFF00;
		...
		for(heapIndx0=0; heapIndx0<4; heapIndx0++){
			for(heapIndx1=0; heapIndx1<4; heapIndx1++){
				...
				NmppsFFTSpec* spec;
				if (nmppsFFT2048InvInitAllocCustom(&spec, nmppsMalloc32, nmppsFree, NMPP_OPTIMIZE_DISABLE )==NMPP_OK){
					t0=clock();
