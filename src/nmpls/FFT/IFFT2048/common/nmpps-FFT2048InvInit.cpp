...
//int nmppsFFT2048InvInitAlloc4888(  NmppsFFTSpec* spec, src int settings)



void nmppsFFT2048InvInitSinCos4888(NmppsFFTSpec* spec)
{
...
int  nmppsFFT2048InvInitAlloc4888( NmppsFFTSpec** specFFT, const void* src, const void* dst,  int settings)
{
	if (settings&NMPP_OPTIMIZE_ALLOC){
		nmppsMallocResetRoute();
		do {
			NmppsFFTSpec* spec;
			if (nmppsFFT2048InvInitAlloc(&spec, src,dst, NMPP_CUSTOM_ALLOC|SKIP_SINCOS)==NMPP_OK){
				nmppsMallocTimerStart();
				nmppsFFT2048Inv((nm32sc*)src, (nm32sc*)dst, spec);
	...
	*specFFT=spec;
	...
	if (!(settings&SKIP_SINCOS)){
		nmppsFFT2048InvInitSinCos4888(spec);
	}	
...
int  nmppsFFT2048InvInitAlloc( NmppsFFTSpec** specFFT, const void* src, const void* dst,  int settings)
{
	return nmppsFFT2048InvInitAlloc4888( specFFT, src, dst, settings);
}
