...
.branch;
	...
	if =0 delayed goto lab_BoolCnv16to1_0 with gr4 >>= 11;
		ar5 = _w_BoolCnv_L with gr5;
	gr4++ noflags;
<lab_BoolCnv16to1_0>
	if > delayed goto lab_BoolCnv16to1_1 with gr5 = true;
		ar1 = _buf_BoolCnv_L;
	ar5 = _w_BoolCnv_G;
	ar1 = _buf_BoolCnv_G;
<lab_BoolCnv16to1_1>
	...
	f1cr = 0fffefffeh;
	gr4--;
	if =0 delayed goto lab_BoolCnv16to1_3 with gr4--;
		WTW_REG(gr5);
<lab_BoolCnv16to1_2>
...
.endrepeat;
	...
	rep 32 wfifo = [ar4++], ftw;
	rep 32 [ar6++] = afifo;
	if > delayed goto lab_BoolCnv16to1_2 with gr4--;
		WTW_REG(gr5);
<lab_BoolCnv16to1_3>
	rep 32 data = [ar0++gr0], ftw with vsum, activate data, vr;
	WTW_REG(gr5);
...
.endrepeat;
	ar4 = ar1 with gr1;
	if =0 delayed goto lab_BoolCnv16to1_5 with gr5 = gr1 << 27;
		gr1 >>= 5;
		gr5;
	rep 32 data = [ar0++gr0] with vsum, activate data, afifo;
	rep 32 [ar4++] = afifo;
<lab_BoolCnv16to1_4>
	if <>0 delayed goto lab_BoolCnv16to1_41 with gr1--;
		ar5 = _mask_table;
	gr1--;
<lab_BoolCnv16to1_41>
	if < goto lab_BoolCnv16to1_42;
<lab_BoolCnv16to1_411>
	if > delayed goto lab_BoolCnv16to1_411 with gr1--;
		gr4 = [ar1++];
		[ar6++] = gr4;
<lab_BoolCnv16to1_42>
	gr4 = [ar1++] with gr5 >>= 27;
	gr5 = [ar5+=gr5];
	gr4 = gr4 and gr5;
	delayed goto lab_BoolCnv16to1_6 with gr4 and gr2;
		[ar6++] = gr4;
		nul;
<lab_BoolCnv16to1_5>
	rep 32 data = [ar0++gr0] with vsum, activate data, afifo;
