...
int nmppsCmpLt_64s8u (nm64s* src1,nm64s* src2, nm8u* dst,  int size)
{
	#ifdef RPC
		int ret;	
		...
		ret = aura_call(n, "nmppsCmpLt_64s8u", &retbuf,  iobuf_src1, iobuf_src2, iobuf_dst, size); 
		if (ret != 0) {
			printf ("bug = %d\r\n",ret);
			BUG(n, "Call:nmppsCmpLt_64s8u failed!"); 
		}
		...
		slog(0, SLOG_INFO, "ARM: Call nmppsCmpLt_64s8u -ok"); 
	#else
