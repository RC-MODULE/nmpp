...
//	nmmtr8s mDst(pDstImg,nHeight,nWidth,nWidth);
//	nmmtr8s mKer(nKerHeight,nKerWidth);
//	nmmtr8s mDstWxH(nKerHeight,nKerWidth);
	...
	for(int y=0; y<nHeight; y++){
        for(int x=0; x<nWidth; x++){
			mtr<nm32s> mSrcWxH(pSrcImg+(y-(nKerHeight>>1))*nWidth+x-(nKerWidth>>1),nKerHeight,nKerWidth,nWidth);
