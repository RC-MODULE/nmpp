...
int nmppsFFT8192Fwd28888Raw(const nm32sc* src, nm32sc* dst, const NmppsFFTSpec* spec)
{
	...
	nm32sc* pN   =(nm32sc*)dst;
	
	nm32sc* pGRe =(nm32sc*)spec->buffer[0];
	nm32sc* pGIm =(nm32sc*)spec->buffer[1];
	nm32sc* pGRaw=(nm32sc*)dst;
	nm32sc* pG   =(nm32sc*)src;


	nm32sc* pHRe =(nm32sc*)spec->buffer[0];
	nm32sc* pHIm =(nm32sc*)spec->buffer[1];
	nm32sc* pHRaw=(nm32sc*)dst;
	nm32sc* pH   =(nm32sc*)src;

	nm32sc* pJRe =(nm32sc*)spec->buffer[0];
	nm32sc* pJIm =(nm32sc*)spec->buffer[1];
	nm32sc* pJRaw=(nm32sc*)dst;
	nm32sc* pJ   =(nm32sc*)src;

	
	nm32sc* pYRe =(nm32sc*)spec->buffer[0];
	nm32sc* pYIm =(nm32sc*)spec->buffer[1];
	...
	//	for(int kk=0; kk<512; kk++){
	//		for(int g=0; g<8; g++){
	//			G[512*(k%16)+kk]+= N[4096*(k%2)+512*g+kk]*	expFFT<8192>(512*g*k,spec->amp[1]);
	//		}
	//	}
	//}
	//z.re= spec->round[1];
	...
	for(int kk=0; kk<512; kk++){
		...
		for(int k2=0; k2<2; k2++){
			...
			for(int k1=0; k1<8; k1++){	
				...
				vsum_data(cosTbl,pGRe+512*k+kk,spec->round[1]);	cosTbl+=8;
				vsum_data(sinTbl,pGIm+512*k+kk,0);				sinTbl+=8;
			}
		}
	}
	cosTblHold=cosTbl;
	...
	for(int kk=0; kk<64; kk++){
		...
		for (int k2=0; k2<16;k2++){
			...
			for (int k1=0; k1<8;k1++){
				...
				vsum_data(cosTbl,pHRe+64*k+kk,spec->round[2]);	cosTbl+=8;
				vsum_data(sinTbl,pHIm+64*k+kk,0);				sinTbl+=8;
			}
		}
	}
	cosTblHold=cosTbl;
	...
	for(int i=0; i<8; i++){
		...
		for (int k2=0; k2<128;k2++){
			...
			for (int k1=0; k1<8;k1++){
				...
				vsum_data(cosTbl,pJRe+8*k+i,spec->round[3]);	cosTbl+=8;
				vsum_data(sinTbl,pJIm+8*k+i,0);					sinTbl+=8;
			}
		}
	}
	...
	for (int k2=0; k2<1024; k2++){
		...
		for (int k1=0; k1<8; k1++){
			...
			vsum_data(cosTbl,pYRe+k,spec->round[4]);	cosTbl+=8;
			vsum_data(sinTbl,pYIm+k,0);					sinTbl+=8;
		}
	}
	//---------------------------------- 4.1 -----------------------------------------	
	for(int i=0;i<8192; i++){
