...
int nmppsCmpNe_16s8um (const nm16s* src1,const  nm16s* src2, nm8u* dst,  int size,  struct NmppsTmpSpec* spec)
{
	...
	ret = aura_call(n, "nmppsCmpNe_16s8um", &retbuf,  iobuf_src1,  iobuf_src2, iobuf_dst, size); 
	if (ret != 0) {
		BUG(n, "Call nmppsCmpNe_16s8um failed!"); 
	}
	...
	slog(3, SLOG_INFO, "ARM: Call nmppsCmpNe_16s8um -ok"); 
