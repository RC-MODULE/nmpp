    ...
    *iaddr1024 = spec_32fc;
    ...
    for(i = 0; i <  8; i++) {
        for(j = 0; j < 64; j = j + 8) {
            ...
            spec_32fc->Buffers[0][i + j].im = sinf(alpha)  * 0.0009765625; // 1/1024 =  * 0.0009765625
            spec_32fc->Buffers[0][i + j].re = cosf(alpha) * 0.0009765625;
            k++;
        }
        k = 0;
    }
    for(i = 0; i < 512; i++) {
        ...
        spec_32fc->Buffers[7][i].im = sinf(alpha);
        spec_32fc->Buffers[7][i].re = cosf(alpha);
    }
    gr1 = 0;
    for(i = 0; i < 8; i++) {
        spec_32fc->Buffers[1][i].im = spec_32fc->Buffers[7][gr1].im;
        spec_32fc->Buffers[1][i].re = spec_32fc->Buffers[7][gr1].re;
        gr1 += 64;
    }
    gr1 = 0;
    for(i = 0; i < 16; i++) {
        spec_32fc->Buffers[2][i].im = spec_32fc->Buffers[7][gr1].im;
        spec_32fc->Buffers[2][i].re = spec_32fc->Buffers[7][gr1].re;
        gr1 += 32;
    }
    gr1 = 0;
    for(i = 0; i < 32; i++) {
        spec_32fc->Buffers[3][i].im = spec_32fc->Buffers[7][gr1].im;
        spec_32fc->Buffers[3][i].re = spec_32fc->Buffers[7][gr1].re;
        gr1 += 16;
    }
    gr1 = 0;
    for(i = 0; i < 64; i++) {
        spec_32fc->Buffers[4][i].im = spec_32fc->Buffers[7][gr1].im;
        spec_32fc->Buffers[4][i].re = spec_32fc->Buffers[7][gr1].re;
        gr1 += 8;
    }
    gr1 = 0;
    for(i = 0; i < 128; i++) {
        spec_32fc->Buffers[5][i].im = spec_32fc->Buffers[7][gr1].im;
        spec_32fc->Buffers[5][i].re = spec_32fc->Buffers[7][gr1].re;
        gr1 += 4;
    }
    gr1 = 0;
    for(i = 0; i < 256; i++) {
        spec_32fc->Buffers[6][i].im = spec_32fc->Buffers[7][gr1].im;
        spec_32fc->Buffers[6][i].re = spec_32fc->Buffers[7][gr1].re;
        gr1 += 2;
    }
    return 0;
}
