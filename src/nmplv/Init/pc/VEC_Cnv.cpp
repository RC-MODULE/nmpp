...
//	leading zero byte and stores the result in the pDstVec buffer.
void VEC_Cnv(
			nm32u*	pSrcVec,	// input buffer		:long Local [Size/4]
...
void VEC_Cnv(		nm2s* pSrcVec, nm4s* pDstVec, int nSize)
{
	int i, j;
	nm64s*pi64Dst = (nm64s*)pDstVec;
	int iSize = nSize >> 5;
...
void VEC_Cnv(		nm2u* pSrcVec, nm4u* pDstVec, int nSize)
{
	int i, j;
	nm64s*pi64Dst = (nm64s*)pDstVec;
...
//******************************************************************************
void VEC_Cnv(nm32s *pSrc, nm4s *pDst, int nSize)
{
    for(int i=0; i<nSize; i++)
    {
        VEC_SetVal(pDst,i,pSrc[i]);
    }
}

void VEC_Cnv(nm16s *pSrc, nm4s *pDst, int nSize)
{
    for(int i=0; i<nSize; i++)
    {
        VEC_SetVal(pDst,i,pSrc[i]);
...
void VEC_Cnv(nm8s *pSrc, nm4s *pDst, int nSize)
{
    for(int i=0; i<nSize; i++)
    {
        VEC_SetVal(pDst,i,pSrc[i]);
