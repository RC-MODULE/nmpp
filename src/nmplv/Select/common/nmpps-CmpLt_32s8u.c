...
int nmppsCmpLt_32s8u (nm32s* src1, nm32s* src2, nm8u* dst,  int size)
{
	#ifdef RPC
		int ret;	
		...
		ret = aura_call(n, "nmppsCmpLt_32s8u", &retbuf,  iobuf_src1,  iobuf_src2, iobuf_dst, size); 
		if (ret != 0) {
			printf ("bug = %d\r\n",ret);
			BUG(n, "Call:nmppsCmpLt_32s8u failed!"); 
		}
		...
		slog(0, SLOG_INFO, "ARM: Call nmppsCmpLt_32s8u -ok"); 
	#else
